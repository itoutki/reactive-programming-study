...
*** application/json ok
*** applicaiton/stream+json ok
*** multipart (file upload) ok
** Header ok
** Cookie ok
** HTTPS ok
** event stream、stream json形式のAPIからの取得（GET・POST）ok
** 直列実行
** 並列実行
** リトライ ok
*** Reactor Core 3.3.3.RELEASEまではReactor ExtraのRetryを使っていた
*** 3.3.4.RELEASEからReactor Core側にRetryが実装された
** キャッシュ
** エラーハンドリング ok
*** ステータスコードごとのハンドリング
*** タイムアウト時のハンドリング
...
=== Spring Boot(Web MVC)の中でWebClientを使いたい場合

* spring-boot-starter-webとspring-boot-starter-webfluxを依存関係に追加する
* 両方が依存関係に存在する場合、Web MVCのauto configurationが行われる。
...
* Spring Bootでは、WebClient.Builder、ReactorResourceFactory、ReactorClientHttpConnectorをBeanとして登録している。(WebClientAutoConfiguration.java、ClientHttpConnectorConfiguration.java)
* また、WebClientをカスタマイズする仕組みとしてWebClientCustomizerを用意しており、 WebClientCustomizerをBeanに登録することでWebClientをカスタマイズできるようになっている。
* そのため、WebClientを独自にBean定義してしまうと、WebClientCustomizerによるカスタマイズが行われない。
...
* 2.3.0.RC1からReactorNettyHttpClientMapperクラスが用意され、HttpClientをカスタマイズする手段が提供されるようになった。
